/**
 * @fileoverview Smart Hostel Management System - Financial Fee Ledger Part 2
 * @module backend/database/feeTransactionLedgerPart2
 * @description Itemized financial transactions for hostel resident accounts 251 to 500.
 * @version 1.0.0
 * @license UNLICENSED
 */
...
const FEE_TRANSACTION_LEDGER_PART2 = [];

for (let i = 251; i <= 500; i++) {
  const isRoomPaid = i % 5 !== 0;
  const isMessPaid = i % 4 !== 0;
  const roomAmount = (i % 2 === 0) ? 55000 : 45000;
  ...
  FEE_TRANSACTION_LEDGER_PART2.push({
    ledgerId: `TXN-P2-${String(i).padStart(4, '0')}`,
