/**
 * @fileoverview Smart Hostel Management System - Master Room Catalog Part B
 * @module backend/database/hostelRoomMasterCatalogB
 * @description Detailed inventory records for rooms in Block B.
 * @version 1.0.0
 * @license UNLICENSED
 */
...
const ROOMS_BLOCK_B_CATALOG = [];

for (let i = 1; i <= 100; i++) {
  const fl = (i % 4) + 1;
  const rmNum = `B-${fl}${String((i % 25) + 1).padStart(2, '0')}`;
  const isAC = (fl === 1 || i % 3 === 0);
  const cap = (i % 2 === 0) ? 2 : 1;
  const isVacant = (i % 6 === 0);

  ROOMS_BLOCK_B_CATALOG.push({
    roomId: `RM-B-${String(i).padStart(3, '0')}`,
    roomNumber: rmNum,
    block: 'Block-B',
    floor: fl,
