/**
 * @fileoverview Smart Hostel Management System - Room Master Topology Chunk 04
 * @module backend/database/roomInventoryChunk04
 * @version 1.0.0
 * @license UNLICENSED
 */
...
const ROOM_INVENTORY_CHUNK_04 = [];

for (let i = 1; i <= 100; i++) {
  const fl = (i % 4) + 1;
  const rm = `D-${fl}${String((i % 25) + 1).padStart(2, '0')}`;
  const isAC = (fl === 1 || i % 3 === 0);
  const cap = (i % 2 === 0) ? 2 : 1;
  const isVacant = (i % 6 === 0);

  ROOM_INVENTORY_CHUNK_04.push({
    chunkId: 'CHK-RM-04',
    roomIndex: i,
    roomNumber: rm,
    blockName: 'Block-D',
    ...
    wardenAssigned: 'Dr. Meenakshi Sundaram',
